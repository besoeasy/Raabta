<template>
  <Transition name="fade">
    ...
    >
      <!-- Header -->
      <div class="absolute top-0 left-0 right-0 p-6 bg-gradient-to-b from-black/50 to-transparent z-10">
        <div class="flex items-center justify-between text-white">
          <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white text-xl font-semibold">
          ...
          <div class="text-2xl font-mono">{{ callDuration }}</div>
        </div>
      </div>
      ...
      <div class="flex-1 relative">
        ...
        <div 
          class="absolute top-20 right-6 w-48 h-36 bg-gray-800 rounded-lg overflow-hidden shadow-2xl border-2 border-white/20"
          v-if="callType === 'video'"
        >
          <video 
            ref="localVideo"
        ...
        >
          <div class="text-center text-white">
            <div class="w-24 h-24 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white text-4xl font-semibold mx-auto mb-6">
              {{ contactName.charAt(0).toUpperCase() }}
            </div>
            <div v-if="callStatus === 'ringing'" class="mb-4">
              <div class="inline-block animate-pulse text-4xl mb-2">📞</div>
            </div>
            <h2 class="text-2xl font-semibold mb-2">{{ contactName }}</h2>
      ...
      <!-- Controls -->
      <div class="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black/50 to-transparent">
        <div class="flex items-center justify-center gap-6">
          <!-- Mute Audio -->
          <button
            @click="toggleAudio"
            :class="[
              'w-14 h-14 rounded-full flex items-center justify-center transition shadow-lg',
              isAudioMuted ? 'bg-red-500 hover:bg-red-600' : 'bg-gray-700 hover:bg-gray-600'
            ]"
            :title="isAudioMuted ? 'Unmute' : 'Mute'"
          >
            <svg v-if="!isAudioMuted" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" viewBox="0 0 20 20" fill="currentColor">
          ...
          >
            <svg v-if="!isVideoMuted" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" viewBox="0 0 20 20" fill="currentColor">
          ...
          <button
            @click="endCall"
            class="w-16 h-16 rounded-full bg-red-500 hover:bg-red-600 flex items-center justify-center transition shadow-lg"
            title="End call"
          >
